...
session_start();
define('version', '2.2.1'); // 2.3.0 test version with theme/plugin URL ZIP installer/remover + file uploader/remover + redesigned settings panel + other fixes and style fixes
...
class wCMS {
	...
	public static function init() {
		...
		wCMS::_logoutAction(); wCMS::_loginAction(); wCMS::_saveAction(); wCMS::_changePasswordAction(); wCMS::_deleteAction(); wCMS::_upgradeAction(); wCMS::_notify(); wCMS::_removeFile(); wCMS::_uploadFile();
	...
	public static function settings() {
		...
		$output .= '</div><div role="tabpanel" class="tab-pane" id="general"><p class="subTitle">Main website title</p><div class="change"><div data-target="config" id="siteTitle" class="editText">' . (wCMS::get('config','siteTitle') != '' ? wCMS::get('config','siteTitle') : '') . '</div></div>';
		$output .= wCMS::getMenuSettings();
		$output .= '<p class="subTitle">Theme</p><div class="form-group"><div class="change"><select class="form-control" name="themeSelect" onchange="fieldSave(\'theme\',this.value,\'config\');">';
		...
		$output .= '</select></div></div><p class="subTitle">Default homepage <small>(what page to show on homepage)</small></p><div class="change"><div data-target="config" id="defaultPage" class="editText">' . wCMS::get('config','defaultPage') . '</div></div><p class="subTitle">Footer</p> <div class="change"><div data-target="blocks" id="footer" class="editText">' . (wCMS::get('blocks','footer')->content != '' ? wCMS::get('blocks','footer')->content : '') . '</div></div></div><div role="tabpanel" class="tab-pane" id="files"><p class="subTitle">Upload</p><div class="change"><form action="' . wCMS::url(wCMS::$currentPage) . '" method="post" enctype="multipart/form-data"><div class="input-group"><input type="file" name="uploadFile" class="form-control"><span class="input-group-btn"><button type="submit" class="btn btn-info">Upload</button></span><input type="hidden" name="token" value="' . wCMS::_generateToken() . '"></div><p class="text-right">Upload location: ' . wCMS::url('uploads') . '</p></form></div><p class="subTitle marginTop20">Remove files</p><div class="change">';
		foreach ($fileList as $file) { $output .= '<a href="' . wCMS::url('?deleteFile='.$file.'&token='.wCMS::_generateToken()).'" class="btn btn-xs btn-danger" onclick="return confirm(\'Delete ' . $file . '?\')" title="Delete file">&times;</a> '.$file.'<p></p>'; }
		$output .= '</div></div><div role="tabpanel" class="tab-pane" id="security"><p class="subTitle">Password</p><div class="change"><form action="' . wCMS::url(wCMS::$currentPage) . '" method="post"><div class="form-group"><input type="password" name="old_password" class="form-control" placeholder="Old password"></div><div class="form-group"><input type="password" name="new_password" class="form-control" placeholder="New password"></div><input type="hidden" name="fieldname" value="password"><input type="hidden" name="token" value="' . wCMS::_generateToken() . '"><button type="submit" class="btn btn-info">Change password</button></form></div><p class="subTitle">Login URL <small>(save your URL: ' . wCMS::url(wCMS::get('config','login')) . ')</small></p><div class="change"><div data-target="config" id="login" class="editText">' . wCMS::get('config','login') . '</div></div></div><div role="tabpanel" class="tab-pane" id="themesAndPlugins"><p class="subTitle">Install or update</p><div class="change"><form action="' . wCMS::url(wCMS::$currentPage) . '" method="post"><div class="form-group"><label class="radio-inline"><input type="radio" name="installLocation" value="themes">Theme</label><label class="radio-inline"><input type="radio" name="installLocation" value="plugins">Plugin</label><p></p><div class="input-group"><input type="text" name="addonURL" class="form-control" placeholder="Paste link/URL to ZIP file"><span class="input-group-btn"><button type="submit" class="btn btn-info">Install/Update</button></span></div></div><input type="hidden" value="true" name="installAddon"><input type="hidden" name="token" value="' . wCMS::_generateToken() . '"></form></div><p class="text-right"><a href="https://github.com/robiso/wondercms-themes#list-of-approved-themes" class="btn btn-info" target="_blank">Theme ZIP links</a> <a href="https://github.com/robiso/wondercms-plugins#approved-plugins" class="btn btn-info" target="_blank">Plugin ZIP links</a></p><p class="subTitle">Remove themes</p><div class="change">';
...
EOT;
	...
	public static function _logoutAction() {
		if (wCMS::$currentPage === 'logout') { unset($_SESSION['l'], $_SESSION['i'], $_SESSION['u'], $_SESSION['token']);wCMS::redirect(); }
	}
	...
	public static function _notify() {
		...
		if ( ! isset($_SESSION['u'])) {$_SESSION['u'] = true; {$repoVersion = wCMS::_getOfficialVersion(); if ($repoVersion != version) { wCMS::alert('info', '<b>Your WonderCMS version is out of date.</b> <form style="display:inline" action="" method="post"><button class="btn btn-info" name="upgrade">Update WonderCMS</button><input type="hidden" name="token" value="' . wCMS::_generateToken() . '"></form><p>Before updating:</p><p>- <a href="https://github.com/robiso/wondercms/wiki/Backup-all-files" target="_blank">backup all files</a></p><p>- <a href="https://www.wondercms.com/whatsnew" target="_blank">check what\'s new</a></p>', true);};}}
	}
	...
	public static function _installThemePlugin() {
		...
		if ($_REQUEST['token'] == wCMS::_generateToken()) {
			...
			if ($installLocation == 'themes' || $installLocation == 'plugins' && ! empty($addonURL)) {
				$zipFile = __DIR__ . '/uploads/ZIPFromURL.zip';
				...
				if ($zip->open($zipFile) != 'true' || (stripos($addonURL,'.zip') != true)) {
					wCMS::_recursiveDelete( __DIR__ . '/uploads/ZIPFromURL.zip'); wCMS::alert('danger', 'Error openning ZIP file.'); wCMS::redirect();
				}
				$zip->extractTo($extractPath); $zip->close(); wCMS::_recursiveDelete( __DIR__ . '/uploads/ZIPFromURL.zip'); wCMS::alert('success', 'Installed successfully.'); wCMS::redirect();
			} else {
	...
	public static function _uploadFile() {
		...
		if ($_REQUEST['token'] == wCMS::_generateToken()) {
			try {
				...
				if ( ! move_uploaded_file($_FILES['uploadFile']['tmp_name'], sprintf(__DIR__ . '/uploads/%s', $_FILES['uploadFile']['name']))) {
					wCMS::alert('danger', 'Failed to move uploaded file.');	wCMS::redirect(wCMS::$currentPage);
				}
				wCMS::alert('success', 'File uploaded successfully.'); wCMS::redirect(wCMS::$currentPage);
	...
	public static function _removeFile() {
		...
		if ($_REQUEST['token'] == wCMS::_generateToken()) {
			...
			foreach ($fileList as $file) {
				if ($_REQUEST['deleteFile'] == $file) { wCMS::_recursiveDelete($filesDirectory . '/' . $file); wCMS::alert('success', 'File removed.'); wCMS::redirect(); }
			}
			$themesDirectory = __DIR__ . '/themes/'; $themeList = array_diff(scandir($themesDirectory), array('..', '.'));
			foreach ($themeList as $theme) {
				if (wCMS::get('config','theme') == $_REQUEST['deleteTheme']) { wCMS::alert('danger', 'Cannot remove currently active theme.'); wCMS::redirect(); }
				elseif ($_REQUEST['deleteTheme'] == $theme) { wCMS::_recursiveDelete($themesDirectory . '/' . $theme . '/'); wCMS::alert('success', 'Theme removed.'); wCMS::redirect(); }
			}
			$pluginsDirectory = __DIR__ . '/plugins/'; $pluginList = array_diff(scandir($pluginsDirectory), array('..', '.'));
			foreach ($pluginList as $plugin) {
				if ($_REQUEST['deletePlugin'] == $plugin) { wCMS::_recursiveDelete($pluginsDirectory . '/' . $plugin . '/'); wCMS::alert('success', 'Plugin removed.'); wCMS::redirect(); }
			}
		}
	}
...
<p class="lightFont">Your website is now powered by WonderCMS.</p>
