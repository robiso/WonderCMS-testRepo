...
session_start();
define('version', '2.3.2'); // 2.3.3 test version
...
class wCMS
{
    ...
    private static function page($key)
    {
    ...
    private static function block($key)
    {
    ...
    private static function asset($location)
    {
    ...
    private static function settings()
    {
        ...
        $output .= '</div></div></div></div><div class="modal-footer clear"><p><small><a href="https://wondercms.com" target="_blank">WonderCMS</a> '. version . ' &bull; <a href="https://wondercms.com/community" target="_blank">Community</a> &bull; <a href="https://github.com/robiso/wondercms/wiki#wondercms-documentation" target="_blank">Documentation</a> &bull; <a href="https://wondercms.com/donate" target="_blank">Donate</a></small></p></div></div></div></div></div>';
...
EOT;
    ...
    private static function newMenuItem($content, $menu, $visibility)
    {
        $conf = 'config';
        $field = 'menuItems';
        ...
        if (! $exist) {
            ...
            if ($slug !== $oldSlug) {
                wCMS::createPage($slug);
                wCMS::deleteAction($oldSlug, false);
            }
        }
    }
    ...
    private static function saveAction()
    {
        ...
        if (isset($_POST['fieldname']) && isset($_POST['content']) && isset($_POST['target']) && isset($_REQUEST['token']) && hash_equals($_REQUEST['token'], wCMS::generateToken())) {
            ...
            if ($target === 'menuItem') {
                wCMS::newMenuItem($content, $menu, $visibility);
            }
    ...
    private static function deleteAction($needle = false, $menu = true)
    {
        if (! $needle) {
    ...
    public static function notFoundView()
    {
        ...
        return ['title' => 'Page not found', 'description' => '', 'keywords' => '', 'content' => '<h4>Sorry, page not found. :(</h4>'];
    }
    ...
    private static function notify()
    {
        ...
        if (! wCMS::$currentPageExists) {
            wCMS::alert('info', '<b>This page (' . wCMS::$currentPage . ') doesn\'t exist yet.</b> Click inside the content below to create it.');
        }
    ...
    private static function getExternalFile($url)
    {
        $ch = curl_init();
    ...
    private static function loadPlugins()
    {
        ...
        if (! is_dir(__DIR__ . '/files/images') || ! is_dir(__DIR__ . '/files/docs') || ! is_dir(__DIR__ . '/files/docs')) {
            mkdir(__DIR__ . '/files');
            mkdir(__DIR__ . '/files/docs');
            mkdir(__DIR__ . '/files/images');
        }
    ...
    private static function createPage($slug = false)
    {
        ...
        if (! $slug) {
            wCMS::newMenuItem(wCMS::slugify(wCMS::$currentPage), null, "show");
        }
    }
    ...
    private static function hook()
    {
        ...
        if (! isset(wCMS::$_listeners[$hookName])) {
            return $args;
        }
        foreach (wCMS::$_listeners[$hookName] as $func) {
            $args = $func($args);
        }
        return $args;
    }

    private static function installThemePlugin()
    {
        ...
        if (hash_equals($_REQUEST['token'], wCMS::generateToken())) {
            ...
            if (in_array($installLocation, $validPaths) && ! empty($addonURL)) {
                ...
                if ($zip->open($zipFile) != 'true' || (stripos($addonURL, '.zip') != true)) {
                    ...
                    wCMS::alert('danger', 'Error openning ZIP file.');
                    wCMS::redirect();
                }
                ...
                wCMS::redirect();
            } elseif (empty($addonURL)) {
                wCMS::alert('danger', 'Error openning ZIP file.');
            } else {
                wCMS::alert('danger', 'Choose between theme or plugin and enter link to ZIP file.');
                wCMS::redirect();
            }
        }
    }
    ...
    private static function removeFile()
    {
        if (! wCMS::$loggedIn) {
            return;
        }
        if (isset($_REQUEST['deleteFile']) || isset($_REQUEST['deleteTheme']) || isset($_REQUEST['deletePlugin']) && isset($_REQUEST['token']) && hash_equals($_REQUEST['token'], wCMS::generateToken())) {
            $fileList = array_slice(scandir(__DIR__ . '/files/'), 2);
            foreach ($fileList as $file) {
                if (isset($_REQUEST['deleteFile']) && $_REQUEST['deleteFile'] == $file) {
                    wCMS::recursiveDelete(__DIR__ . '/files/' . $file);
                    wCMS::alert('success', 'File removed.');
                    wCMS::redirect();
                }
            }
            $fileImagesList = array_slice(scandir(__DIR__ . '/files/images/'), 2);
            foreach ($fileImagesList as $fileImage) {
                if (isset($_REQUEST['deleteFile']) && $_REQUEST['deleteFile'] == $fileImage) {
                    wCMS::recursiveDelete(__DIR__ . '/files/images/' . $fileImage);
                    wCMS::alert('success', 'File removed.');
                    wCMS::redirect();
                }
            }
            ...
            foreach ($fileDocsList as $fileDoc) {
                if (isset($_REQUEST['deleteFile']) && $_REQUEST['deleteFile'] == $fileDoc) {
                    wCMS::recursiveDelete(__DIR__ . '/files/docs/' . $fileDoc);
                    wCMS::alert('success', 'File removed.');
                    wCMS::redirect();
                }
            }
            ...
            foreach ($themeList as $theme) {
                if (isset($_REQUEST['deleteTheme']) && wCMS::get('config', 'theme') == $_REQUEST['deleteTheme']) {
                    wCMS::alert('danger', 'Cannot remove currently active theme.');
                    wCMS::redirect();
                } elseif (isset($_REQUEST['deleteTheme']) && $_REQUEST['deleteTheme'] == $theme) {
                    wCMS::recursiveDelete(__DIR__ . '/themes/' . $theme . '/');
                    wCMS::alert('success', 'Theme removed.');
                    wCMS::redirect();
                }
            }
            $pluginList = array_slice(scandir(__DIR__ . '/plugins'), 2);
            foreach ($pluginList as $plugin) {
                if (isset($_REQUEST['deletePlugin']) && $_REQUEST['deletePlugin'] == $plugin) {
                    wCMS::recursiveDelete(__DIR__ . '/plugins/' . $plugin . '/');
                    wCMS::alert('success', 'Plugin removed.');
                    wCMS::redirect();
                }
            }
        }
