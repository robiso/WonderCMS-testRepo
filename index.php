...
EOT;
	...
	private static function httpsSwitch()
	{
		if (! wCMS::$loggedIn || ! isset($_POST['httpsSwitch'])) {
			return;
		}
		if (hash_equals($_POST['token'], wCMS::generateToken()) && $_POST['httpsSwitch'] == 'y') {
			...
			wCMS::alert('success', 'HTTPS and secure mode turned ON.');
			wCMS::redirect();
		} elseif (hash_equals($_POST['token'], wCMS::generateToken()) && $_POST['httpsSwitch'] == 'n') {
			$contents = wCMS::getExternalFile('https://raw.githubusercontent.com/robiso/wondercms/master/.htaccess');
			if ($contents) {
				file_put_contents('.htaccess', trim($contents));
			}
			wCMS::alert('success', 'HTTPS and secure mode turned OFF.');
			wCMS::redirect();
		}
	}
