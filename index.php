...
class wCMS {
	...
	public static function settings() {
		...
		if (wCMS::$currentPageExists) {
			...
			$output .= 'This page doesn\'t exist. After this page is created, more settings will be displayed here.';
		}
...
EOT;
	...
	public static function _loginAction() {
		...
		if (password_verify($password, wCMS::get('config','password'))) {
			$_SESSION['l'] = true; $_SESSION['i'] = __DIR__; wCMS::redirect();
		}
		wCMS::alert('danger', 'Wrong password.'); wCMS::redirect(wCMS::get('config','login'));
	}
	public static function _logoutAction() {
		if (wCMS::$currentPage === 'logout' && $_REQUEST['token'] == wCMS::_generateToken()) {
			unset($_SESSION['l'], $_SESSION['i'], $_SESSION['u'], $_SESSION['token']); wCMS::redirect();
		}
	}
	public static function _newMenuItem($content, $menu, $visibility) {
		...
		if ( ! $exist) {
			$db = wCMS::db();
			$slug.= ($menu) ? "-" . $menuCount : "";
			foreach($db->config->{$field} as $key=>$value) if ($value->slug == $slug) $slug.= "-extra";
	...
	public static function _saveAction() {
		if ( ! wCMS::$loggedIn) return;
		if (isset($_POST['fieldname']) && isset($_POST['content']) && isset($_POST['target']) && isset($_REQUEST['token']) && ($_REQUEST['token'] == wCMS::_generateToken())) {
			...
			if ($target === 'config') wCMS::set('config', $fieldname, $content);
			elseif ($target === 'blocks') wCMS::set('blocks', $fieldname, 'content', $content);
			elseif ($target === 'pages') {
				if ( ! isset(wCMS::get('pages')->{wCMS::$currentPage})) wCMS::_createPage();
	...
	public static function _changePasswordAction() {
		...
		if ($_SESSION['token'] === $_REQUEST['token'] && ($_REQUEST['token'] == wCMS::_generateToken())) {
			if ( ! password_verify($_POST['old_password'], wCMS::get('config','password'))) {
				wCMS::alert('danger', 'Wrong password.'); wCMS::redirect(wCMS::$currentPage);
			}
			if (strlen($_POST['new_password']) < 4) {
				wCMS::alert('danger', 'Password must be longer than 4 characters.'); wCMS::redirect(wCMS::$currentPage);
			}
	...
	public static function _deleteAction($needle = false, $menu = true) {
		if ( ! $needle) {
			if ( ! wCMS::$loggedIn || ! isset($_GET['delete']) || ($_REQUEST['token'] != wCMS::_generateToken())) return;
			$needle = $_GET['delete'];
		}
		$db=wCMS::db();
	...
	public static function _notify() {
		...
		if (wCMS::get('config','login') === 'loginURL') wCMS::alert('warning', 'Change the default admin login URL. (<i>Settings -> Security</i>)', true);
		if (password_verify('admin', wCMS::get('config','password'))) wCMS::alert('danger', 'Change the default password. (<i>Settings -> Security</i>)', true);
		$repoVersion = wCMS::_getOfficialVersion();
	...
	public static function _loadPlugins() {
		...
		if ( ! is_dir(__DIR__ . '/files/images') || ! is_dir(__DIR__ . '/files/docs') || ! is_dir(__DIR__ . '/files/docs')) {
			mkdir(__DIR__ . '/files'); mkdir(__DIR__ . '/files/docs'); mkdir(__DIR__ . '/files/images');
		}
	...
	public static function _installThemePlugin() {
		...
		if ($_REQUEST['token'] == wCMS::_generateToken()) {
			...
			if ($installLocation == 'themes' || $installLocation == 'plugins' && ! empty($addonURL)) {
				...
				$zip->close();
				wCMS::_recursiveDelete(__DIR__ . '/files/ZIPFromURL.zip'); wCMS::alert('success', 'Installed successfully.'); wCMS::redirect();
			} else {
	...
	public static function _removeFile() {
		if ( ! wCMS::$loggedIn) return;
		if (isset($_REQUEST['deleteFile']) || isset($_REQUEST['deleteTheme']) || isset($_REQUEST['deletePlugin']) && isset($_REQUEST['token']) && $_REQUEST['token'] == wCMS::_generateToken()) {
			$fileList = array_slice(scandir(__DIR__ . '/files/'), 2);
			foreach ($fileList as $file) {
				if (isset($_REQUEST['deleteFile']) && $_REQUEST['deleteFile'] == $file) {
					wCMS::_recursiveDelete(__DIR__ . '/files/' . $file); wCMS::alert('success', 'File removed.'); wCMS::redirect();
				}
			}
			$fileImagesList = array_slice(scandir(__DIR__ . '/files/images/'), 2);
			foreach ($fileImagesList as $fileImage) {
				if (isset($_REQUEST['deleteFile']) && $_REQUEST['deleteFile'] == $fileImage) {
					wCMS::_recursiveDelete(__DIR__ . '/files/images/' . $fileImage); wCMS::alert('success', 'File removed.'); wCMS::redirect();
				}
			}
			$fileDocsList = array_slice(scandir(__DIR__ . '/files/docs/'), 2);
			foreach ($fileDocsList as $fileDoc) {
				if (isset($_REQUEST['deleteFile']) && $_REQUEST['deleteFile'] == $fileDoc) {
					wCMS::_recursiveDelete(__DIR__ . '/files/docs/' . $fileDoc); wCMS::alert('success', 'File removed.'); wCMS::redirect();
				}
			}
			$themeList = array_slice(scandir(__DIR__ . '/themes'), 2);
			foreach ($themeList as $theme) {
				if (isset($_REQUEST['deleteTheme']) && wCMS::get('config','theme') == $_REQUEST['deleteTheme']) {
					wCMS::alert('danger', 'Cannot remove currently active theme.'); wCMS::redirect();
				}
				elseif (isset($_REQUEST['deleteTheme']) && $_REQUEST['deleteTheme'] == $theme) {
					wCMS::_recursiveDelete(__DIR__ . '/themes/' . $theme . '/'); wCMS::alert('success', 'Theme removed.'); wCMS::redirect();
				}
			}
			$pluginList = array_slice(scandir(__DIR__ . '/plugins'), 2);
			foreach ($pluginList as $plugin) {
				if (isset($_REQUEST['deletePlugin']) && $_REQUEST['deletePlugin'] == $plugin) {
					wCMS::_recursiveDelete(__DIR__ . '/plugins/' . $plugin . '/'); wCMS::alert('success', 'Plugin removed.'); wCMS::redirect();
				}
			}
		}
	}
