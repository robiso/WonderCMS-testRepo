...
class wCMS
{
	...
	private static function getMenuSettings()
	{
		...
		$end = key($items);
		$output = '<p class="subTitle">Menu</p><div><div id="menuSettings" class="container-fluid"><a class="menu-item-add btn btn-info" data-toggle="tooltip" title="Add new page">Add page</a><p></p>';
	...
	public static function settings()
	{
		...
		$output .= '</div><div role="tabpanel" class="tab-pane" id="general">'.wCMS::getMenuSettings().'<p class="subTitle">Website title</p><div class="change"><div data-target="config" id="siteTitle" class="editText">' . (wCMS::get('config', 'siteTitle') != '' ? wCMS::get('config', 'siteTitle') : '') . '</div></div><p class="subTitle">Theme</p><div class="form-group"><div class="change"><select class="form-control" name="themeSelect" onchange="fieldSave(\'theme\',this.value,\'config\');">';
		...
		$output .= '</div></div></div></div><div class="modal-footer clear"><p><small><a href="https://wondercms.com" target="_blank">WonderCMS</a> '. version . ' &bull; <a href="https://wondercms.com/themes" target="_blank">Get themes</a> &bull; <a href="https://wondercms.com/plugins" target="_blank">Get plugins</a> &bull; <a href="https://wondercms.com/community" target="_blank">Community</a> &bull; <a href="https://github.com/robiso/wondercms/wiki#wondercms-documentation" target="_blank">Documentation</a> &bull; <a href="https://wondercms.com/donate" target="_blank">Donate</a></small></p></div></div></div></div></div>';
...
EOT;
			...
			if (hash_equals($_REQUEST['token'], wCMS::generateToken())) {
				...
				foreach ($fileList as $file) {
					if (isset($_REQUEST['deleteFile']) && $_REQUEST['deleteFile'] == $file) {
						...
						wCMS::alert('success', 'File deleted.');
						wCMS::redirect();
					}
				}
				$themeList = array_slice(scandir(__DIR__ . '/themes'), 2);
				foreach ($themeList as $theme) {
					if (isset($_REQUEST['deleteTheme']) && wCMS::get('config', 'theme') == $_REQUEST['deleteTheme']) {
						...
						wCMS::alert('success', 'Theme deleted.');
						wCMS::redirect();
					}
				}
				$pluginList = array_slice(scandir(__DIR__ . '/plugins'), 2);
				foreach ($pluginList as $plugin) {
					if (isset($_REQUEST['deletePlugin']) && $_REQUEST['deletePlugin'] == $plugin) {
						...
						wCMS::alert('success', 'Plugin deleted.');
						wCMS::redirect();
					}
				}
			 }
	...
	private static function backupAction()
	{
		...
		if (! empty($backupList)) {
			wCMS::alert('danger', 'Delete backup files. (<i>Settings -> Files -> Delete files</i>)');
		}
